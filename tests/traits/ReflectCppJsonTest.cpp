...
TEST(ReflectCppTest, BasicClaims) {
	...
	const auto array = jwt::basic_claim<jwt::traits::reflectcpp_json>(std::set<std::string>{"string", "string"});
...
TEST(ReflectCppTest, VerifyTokenHS256) {
	...
	const auto decoded = jwt::decode<jwt::traits::reflectcpp_json>(token);
