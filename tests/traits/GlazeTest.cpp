...
TEST(GlazeTest, BasicClaims) {
	const auto string = jwt::basic_claim<jwt::traits::glaze_json>(jwt::traits::glaze_json::string_type("string"));
	ASSERT_EQ(string.get_type(), jwt::json::type::string);

	const auto array =
		jwt::basic_claim<jwt::traits::glaze_json>(std::set<jwt::traits::glaze_json::string_type>{"string", "string"});
	ASSERT_EQ(array.get_type(), jwt::json::type::array);

	jwt::traits::glaze_json::value_type jvi = 159816816;
	const auto integer = jwt::basic_claim<jwt::traits::glaze_json>(jvi);
	ASSERT_EQ(integer.get_type(), jwt::json::type::integer);
}

TEST(GlazeTest, AudienceAsString) {
	jwt::traits::glaze_json::string_type token =
		"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJ0ZXN0In0.WZnM3SIiSRHsbO3O7Z2bmIzTJ4EC32HRBKfLznHhrh4";
	auto decoded = jwt::decode<jwt::traits::glaze_json>(token);
...
TEST(GlazeTest, SetArray) {
	...
	auto token = jwt::create<jwt::traits::glaze_json>()
					 .set_payload_claim("test", jwt::basic_claim<jwt::traits::glaze_json>(vect.begin(), vect.end()))
					 .sign(jwt::algorithm::none{});
	ASSERT_EQ(token, "eyJhbGciOiJub25lIn0.eyJ0ZXN0IjpbMTAwLDIwLDEwXX0.");
}

TEST(GlazeTest, SetObject) {
	jwt::traits::glaze_json::value_type value;
	ASSERT_TRUE(jwt::traits::glaze_json::parse(value, "{\"api-x\": [1]}"));
	jwt::basic_claim<jwt::traits::glaze_json> object(value);
	ASSERT_EQ(object.get_type(), jwt::json::type::object);

	auto token =
		jwt::create<jwt::traits::glaze_json>().set_payload_claim("namespace", object).sign(jwt::algorithm::hs256("test"));
	ASSERT_EQ(token,
...
TEST(GlazeTest, VerifyTokenHS256) {
	...
	const auto decoded_token = jwt::decode<jwt::traits::glaze_json>(token);
	const auto verify =
		jwt::verify<jwt::traits::glaze_json>().allow_algorithm(jwt::algorithm::hs256{"secret"}).with_issuer("auth0");
	verify.verify(decoded_token);
}

TEST(GlazeTest, VerifyTokenExpirationValid) {
	const auto token = jwt::create<jwt::traits::glaze_json>()
						   .set_issuer("auth0")
...
TEST(GlazeTest, VerifyArray) {
	jwt::traits::glaze_json::string_type token = "eyJhbGciOiJub25lIn0.eyJ0ZXN0IjpbMTAwLDIwLDEwXX0.";
	const auto decoded_token = jwt::decode<jwt::traits::glaze_json>(token);

	std::vector<int64_t> vect = {100, 20, 10};
	jwt::basic_claim<jwt::traits::glaze_json> array_claim(vect.begin(), vect.end());
	const auto verify =
		jwt::verify<jwt::traits::glaze_json>().allow_algorithm(jwt::algorithm::none{}).with_claim("test", array_claim);
...
TEST(GlazeTest, VerifyObject) {
	...
	jwt::traits::glaze_json::value_type value;
	ASSERT_TRUE(jwt::traits::glaze_json::parse(value, "{\"api-x\": [1]}"));
	jwt::basic_claim<jwt::traits::glaze_json> object_claim(value);
	const auto verify = jwt::verify<jwt::traits::glaze_json>()
