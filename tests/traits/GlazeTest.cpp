...
TEST(GlazeTest, BasicClaims) {
	...
	jwt::traits::glaze::value_type jvi = 159816816;
	const auto integer = jwt::basic_claim<jwt::traits::glaze>(jvi);
...
TEST(GlazeTest, SetObject) {
	jwt::traits::glaze::value_type value;
	ASSERT_TRUE(jwt::traits::glaze::parse(value, "{\"api-x\": [1]}"));
...
TEST(GlazeTest, VerifyObject) {
	...
	jwt::traits::glaze::value_type value;
	ASSERT_TRUE(jwt::traits::glaze::parse(value, "{\"api-x\": [1]}"));
	jwt::basic_claim<jwt::traits::glaze> object_claim(value);
