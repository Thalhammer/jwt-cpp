...
TEST(PicoJsonTest, BasicClaims) {
	const auto string =
		jwt::basic_claim<jwt::traits::kazuho_picojson>(jwt::traits::kazuho_picojson::string_type("string"));
	ASSERT_EQ(string.get_type(), jwt::json::type::string);

	const auto array = jwt::basic_claim<jwt::traits::kazuho_picojson>(
		std::set<jwt::traits::kazuho_picojson::string_type>{"string", "string"});
	ASSERT_EQ(array.get_type(), jwt::json::type::array);

	const auto integer = jwt::basic_claim<jwt::traits::kazuho_picojson>(
		jwt::traits::kazuho_picojson::value_type(jwt::traits::kazuho_picojson::integer_type(159816816)));
	ASSERT_EQ(integer.get_type(), jwt::json::type::integer);
}

TEST(PicoJsonTest, AudienceAsString) {
...
TEST(PicoJsonTest, SetArray) {
	std::vector<int64_t> vect = {100, 20, 10};
	auto token =
		jwt::create<jwt::traits::kazuho_picojson>()
			.set_payload_claim("test", jwt::basic_claim<jwt::traits::kazuho_picojson>(vect.begin(), vect.end()))
			.sign(jwt::algorithm::none{});
	ASSERT_EQ(token, "eyJhbGciOiJub25lIn0.eyJ0ZXN0IjpbMTAwLDIwLDEwXX0.");
}

TEST(PicoJsonTest, SetObject) {
	std::istringstream iss{"{\"api-x\": [1]}"};
	jwt::basic_claim<jwt::traits::kazuho_picojson> object;
	iss >> object;
...
TEST(PicoJsonTest, VerifyTokenHS256) {
	jwt::traits::kazuho_picojson::string_type token =
		"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXUyJ9.eyJpc3MiOiJhdXRoMCJ9.AbIJTDMFc7yUa5MhvcP03nJPyCPzZtQcGEp-zWfOkEE";

	const auto decoded_token = jwt::decode<jwt::traits::kazuho_picojson>(token);
	const auto verify = jwt::verify<jwt::traits::kazuho_picojson>()
							.allow_algorithm(jwt::algorithm::hs256{"secret"})
							.with_issuer("auth0");
	verify.verify(decoded_token);
}

TEST(PicoJsonTest, VerifyTokenExpirationValid) {
	const auto token = jwt::create<jwt::traits::kazuho_picojson>()
						   .set_issuer("auth0")
...
TEST(PicoJsonTest, VerifyTokenExpirationInValid) {
	const auto token = jwt::create<jwt::traits::kazuho_picojson>()
						   .set_issuer("auth0")
						   .set_issued_now()
...
TEST(PicoJsonTest, VerifyTokenExpired) {
	const auto token = jwt::create<jwt::traits::kazuho_picojson>()
						   .set_issuer("auth0")
...
TEST(PicoJsonTest, VerifyArray) {
	jwt::traits::kazuho_picojson::string_type token = "eyJhbGciOiJub25lIn0.eyJ0ZXN0IjpbMTAwLDIwLDEwXX0.";
	const auto decoded_token = jwt::decode<jwt::traits::kazuho_picojson>(token);

	std::vector<int64_t> vect = {100, 20, 10};
	jwt::basic_claim<jwt::traits::kazuho_picojson> array_claim(vect.begin(), vect.end());
	const auto verify = jwt::verify<jwt::traits::kazuho_picojson>()
...
TEST(PicoJsonTest, VerifyObject) {
	jwt::traits::kazuho_picojson::string_type token =
		"eyJhbGciOiJIUzI1NiJ9.eyJuYW1lc3BhY2UiOnsiYXBpLXgiOlsxXX19.F8I6I2RcSF98bKa0IpIz09fRZtHr1CWnWKx2za-tFQA";
	const auto decoded_token = jwt::decode<jwt::traits::kazuho_picojson>(token);

	jwt::basic_claim<jwt::traits::kazuho_picojson> object_claim;
	std::istringstream iss{"{\"api-x\": [1]}"};
	iss >> object_claim;
	const auto verify = jwt::verify<jwt::traits::kazuho_picojson>()
